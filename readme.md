Author: https://t.me/alexhehenotcoin
...
### ВАЖНО
C 07.01 сессия с браузера держится 2 часа.
Для того, чтобы все работало, необходимо проделать действия с 3 пункта.
Ограничения связаны с специальным webapp url(url с event), который невозможно сгенерировать не имея активную сессию телеграмма(скриптом в браузере это сделать точно нельзя).
